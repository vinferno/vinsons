...
exports.reducer = function(config) {
    ...
    if (setup.name) {
        ...
        if (!fs.existsSync(`./${setup.name}`)){
            fs.mkdirSync(`./${setup.name}`);
            fs.writeFile(`./${setup.name}/${setup.name}.module.ts`, entityModule.createModule(setup), function (err) {
                if (err) throw err;
                console.log(`./${setup.name}.module.ts`.blue, 'File is created successfully.');
            });
            fs.writeFile(`./${setup.name}/${setup.name}.reducer.ts`, entityReducer.createReducer(setup), function (err) {
                if (err) throw err;
                console.log(`./${setup.name}.reducer.ts`.blue, 'File is created successfully.');
            });
            fs.writeFile(`./${setup.name}/${setup.name}.actions.ts`, entityActions.createActions(setup), function (err) {
                if (err) throw err;
                console.log(`./${setup.name}.actions.ts`.blue, 'File is created successfully.');
            });
            fs.writeFile(`./${setup.name}/${setup.name}.component.ts`, entityComponent.createComponent(setup), function (err) {
                if (err) throw err;
                console.log(`./${setup.name}.component.ts`.blue, 'File is created successfully.');
            });
            fs.writeFile(`./${setup.name}/${setup.name}.component.html`, entityHtml.createHtml(setup), function (err) {
                if (err) throw err;
                console.log(`./${setup.name}.component.html`.blue, 'File is created successfully.');
            });
            fs.writeFile(`./${setup.name}/${setup.name}.component.scss`, '', function (err) {
                if (err) throw err;
                console.log(`./${setup.name}.component.scss`.blue, 'File is created successfully.');
            });
